...
class Loggers():
    ...
    def __init__(self, save_dir=None, weights=None, opt=None, hyp=None, logger=None, include=LOGGERS):
        ...
        # Check if this is a segmentation model by checking if segment.val is imported
        # or by checking model type if available
        try:
            from utils.segment.metrics import KEYS, BEST_KEYS
            # Use segment keys if available (for segmentation models)
            self.keys = KEYS
            self.best_keys = BEST_KEYS
        except ImportError:
            # Fallback to detection keys
            self.keys = [
                'train/box_loss',
                'train/obj_loss',
        ...
        if wandb and 'wandb' in self.include:
            ...
            if Version(wandb.__version__) >= Version('0.12.11'):
